...
def just_conv(img_name,sigma=None,FWHM_orign=None,FWHM_desinate=None):
	# FWHM_orign=3.01
	# FWHM_desinate=20.36
	if FWHM_orign and FWHM_desinate:
		...
		print(sigma)
		kernel_name = Gaussian2DKernel(sigma)
	elif sigma:
	...
	try:
		...
		hdul[1].data=convolved_kernel
	if '/' in img_name:
		full_path=new_path(img_name,'Conv_')
...
def ISO_Calibrate_List(folder_name,BAO_Fit_Name):
	m=folder_file_name(folder_name)
	try:
	...
	for z in m:
		...
		location=folder_name+'/'+img_name
		fluxscaleindex=Flux_Scale_Index(IPHA=location,BAO=BAO_Fit_Name)
		ISO_Calibrate(location,fluxscaleindex)
		full_name=new_path(img_name,'ISO_Cal_')
		ISO_Cal_Location=folder_name+'/'+full_name
		os.system('mv %s IPHAS_ISO_Cal' %ISO_Cal_Location)

def Flux_Scale_Index(IPHA='r414157-3.fits',BAO='L20160919_04629_001901+6501_180S_S2_178471.fits'):
	...
	FluxScaleIndex=format2csv.Flux_Scale_Index_From_Cat(difference_cat)
...
#ISO_Calibrate_List('IPHAS_PSFex_Convolve','BAO/L2016.fits')
#Reverse_Image_List('IPHAS_ISO_Cal')
