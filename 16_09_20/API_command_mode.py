...
#You must already install package above(pip install astropy numpy request,
#pip install request urllib.)
#When you encounter a error, It perhaps because the network connection 
#between your PC and astrometry api is weak.
#The code isn't use friendly and only have a few functions like upload 
#file or downloading result file.For more information you can visit
#https://nova.astrometry.net/api_help
#If you have any problem when using this code, please contact me by 
#zhuangrui0452@sina.com
#This code producer: Rui Zhuang
...
#########################################################################
'''you should run this by command line but not text editor
like "./API_command_mode_url.py --folder=up_load" which the name,up_load
, is a folder name'''
if __name__ == '__main__':
    import optparse
    parser = optparse.OptionParser()
    parser.add_option('--folder', dest='folder_name', type=str,
		help='specify the folder which contains fits needed to be upl')
    opt,args = parser.parse_args()
    
    if opt.folder_name is None:
        parser.print_help()
        print('You must specify the folder name')
        sys.exit(-1)
 
    r = requests.post('http://nova.astrometry.net/api/login', 
		data={'request-json': json.dumps({"apikey": "wsjkkufuuwxvjovl"})})
    m=r.text
    dic=json.loads(m)
    session=dic['session']
    ...
    file_name_dic=folder_file_name(opt.folder_name)
    for fn in file_name_dic:
	    location=opt.folder_name+'/'+fn
